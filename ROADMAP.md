...
| todo | Label latest event-fade no-key bundle | human | Latest bundle: `/tmp/event_fade_no_key_review_bundle_20260617_032443`. It has 121 rows: 27 proxy candidates, 19 proxy-context controls, 8 direct rows, 67 ambiguous rows, 0 triggers, and 117 missing machine event times. Start with `review_guide.md`, then fill `review_template.csv` for the 20 queued rows with `review_status`, `reviewed_by`, `reviewed_at`, `human_label`, and any confirmed `human_event_time*` fields. |
