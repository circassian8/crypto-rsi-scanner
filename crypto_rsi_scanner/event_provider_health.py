...
class HealthCheckedProvider:
    ...
    def __init__(self, provider: Any, *, cfg: EventProviderHealthConfig) -> None:
    ...
    def search(
        ...
        *,
        ...
        try:
            ...
            record_provider_failure(self.name, exc, cfg=self.cfg, now=now)
        ...
        if warnings and int(getattr(result, "provider_fetch_count", 0) or 0) > 0:
            record_provider_failure(self.name, warnings[0], cfg=self.cfg, now=now)
        else:
            record_provider_success(self.name, cfg=self.cfg, now=now)
        return result
