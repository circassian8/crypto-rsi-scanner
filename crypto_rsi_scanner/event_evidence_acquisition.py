...
def _select_requests(
    ...
    *,
    ...
    for near in near_misses:
        ...
        if request and request.acquisition_id not in seen:
            requests.append(request)
            seen.add(request.acquisition_id)
...
def _request_from_row(
    ...
    *,
    ...
    if not query_plan:
        return None
    hypothesis_id = str(row.get("hypothesis_id") or request.opportunity_id or "").strip() or None
    core_id = str(row.get("core_opportunity_id") or row.get("aggregated_candidate_id") or "").strip() or None
    incident_id = str(row.get("incident_id") or "").strip() or None
    source_pack = str(row.get("source_pack") or planner.source_pack or request.source_pack or "market_anomaly_pack")
    return EvidenceAcquisitionRequest(
        acquisition_id=_acquisition_id(request.opportunity_id, hypothesis_id, source_pack),
...
def _attach_result_to_hypothesis(item: object, result: EvidenceAcquisitionResult | None) -> object:
    ...
    return replace(item, score_components=components, validation_reasons=validation_reasons, warnings=warnings)
...
def _finalize_result(
    ...
    *,
    ...
    after_score = _float(getattr(after, "opportunity_score_final", None)) or _float(after_components.get("opportunity_score_final")) or _float(getattr(after, "hypothesis_score", None)) or result.opportunity_score_before
    after_level = str(getattr(after, "opportunity_level", "") or after_components.get("opportunity_level") or result.opportunity_level_before)
    ...
    if result.accepted_evidence and (after_score > result.opportunity_score_before or after_level != result.opportunity_level_before):
        status = "upgraded"
        reason = "accepted_source_pack_evidence"
        no_upgrade = None
    elif result.accepted_evidence:
        status = "unchanged"
        no_upgrade = "accepted_evidence_did_not_change_quality_verdict"
    elif result.status in {EvidenceAcquisitionStatus.FAILED_SOFT.value, EvidenceAcquisitionStatus.PROVIDER_UNAVAILABLE.value, EvidenceAcquisitionStatus.PROVIDER_BACKOFF.value}:
        status = "failed"
    return replace(
        result,
