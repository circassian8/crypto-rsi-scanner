...
def initialize_cryptopanic_provider(
    self: Any,
    path: str | Path | None,
    *,
    required: bool = False,
...
def fetch_cryptopanic_events(self: Any, start: datetime, end: datetime) -> list[RawDiscoveredEvent]:
    self.last_warnings = ()
...
def fetch_cryptopanic_live_events(self: Any, start: datetime, end: datetime) -> list[RawDiscoveredEvent]:
...
def build_cryptopanic_request_url(
    self: Any,
    token: str,
    *,
...
def cryptopanic_request_cache_key(self: Any, *, currencies: str, page: int) -> tuple[Any, ...]:
...
def cryptopanic_process_request_cache_key(
    self: Any,
...
def cryptopanic_quota_skip_reason(self: Any, *, now: datetime) -> str | None:
    if self.request_ledger_path is None:
        return None
...
def cryptopanic_respect_min_interval(self: Any, *, now: datetime) -> None:
...
def record_cryptopanic_request(
    self: Any,
    *,
