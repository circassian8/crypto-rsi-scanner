...
def _append_system_health_detail_sections(
    ...
    *,
    ...
    if requested_profile and not rows.runs and rows.legacy_available and not include_legacy_artifacts:
...
def _stored_hypothesis_summary_lines(hypotheses: list[dict[str, Any]]) -> list[str]:
    lines = [
        ...
        + f" (legacy={_legacy_hypothesis_count(hypotheses)})",
...
def _legacy_hypothesis_count(hypotheses: list[dict[str, Any]]) -> int:
    return sum(
...
def _append_signal_quality_lines(lines: list[str], rows: _DailyBriefRows, context: _DailyBriefContext) -> None:
    ...
    lines.extend(_legacy_quality_conflict_lines(_legacy_quality_conflicts(rows.alerts)[:8]))
