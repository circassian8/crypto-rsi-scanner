...
def _api_quality_conflicts(rows: Iterable[Mapping[str, Any]]) -> list[Mapping[str, Any]]:
    out = []
    for row in rows:
        classification = str(
...
def _api_quality_conflict_lines(rows: Iterable[Mapping[str, Any]]) -> list[str]:
    items = list(rows)
    if not items:
        return ["- none"]
    lines: list[str] = []
...
__all__ = (
    ...
    '_api_quality_conflicts',
    '_api_quality_conflict_lines',
