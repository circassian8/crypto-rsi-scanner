"""Notification Checks for the artifact doctor."""
...
def _core_has_fresh_rows(opportunity: event_core_opportunities.CoreOpportunity) -> bool:
    return any(
        not event_alpha_artifacts.is_api_row(row)
...
def _quality_missing_summary(
    *,
    ...
    legacy_missing = sum(1 for row in missing_rows if event_alpha_artifacts.is_api_row(row))
    fresh_hypothesis_missing = sum(
        1
        for row in hypothesis_rows
        if event_alpha_quality_fields.missing_top_level_quality_fields(row)
        and not event_alpha_artifacts.is_api_row(row)
    )
    ...
    return {
        ...
        "non_api_quality_missing": max(0, len(missing_rows) - legacy_missing),
    }
...
def _quality_route_conflicts(alerts: Iterable[Mapping[str, Any]], *, legacy: bool) -> int:
    count = 0
    for row in alerts:
        is_api = event_alpha_artifacts.is_api_row(row)
        if legacy != is_api:
            continue
...
def _missing_final_route_rows(alerts: Iterable[Mapping[str, Any]], *, legacy: bool | None = None) -> int:
    count = 0
    for row in alerts:
        if legacy is not None and event_alpha_artifacts.is_api_row(row) != legacy:
            continue
        classification = event_alpha_alert_store.classify_alert_snapshot(row)
