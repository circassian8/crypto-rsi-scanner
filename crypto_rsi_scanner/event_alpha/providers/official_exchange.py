...
def normalize_official_exchange_event(
    ...
    *,
    ...
    return {
        ...
        "run_mode": run_mode,
        "run_id": run_id,
        "provider_generation_id": item.get("provider_generation_id"),
        "provider_request_succeeded": item.get("provider_request_succeeded"),
        "provider_source_artifact": item.get("provider_source_artifact"),
        "request_ledger_path": item.get("request_ledger_path"),
        "provider": provider,
        "exchange": exchange,
...
def _candidate_rows_for_event(event: Mapping[str, Any], item: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for index, symbol in enumerate(symbols):
        ...
        row = {
            ...
            "provider_generation_id": event.get("provider_generation_id"),
            "provider_request_succeeded": event.get("provider_request_succeeded"),
            "provider_source_artifact": event.get("provider_source_artifact"),
            "request_ledger_path": event.get("request_ledger_path"),
