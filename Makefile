...
help:
	...
	@echo "  make event-alpha-notify-go-no-go PROFILE=notify_no_key  Summarize notification send go/no-go checks"
	...
	@echo "  make event-alpha-notification-inbox PROFILE=notify_no_key  Print unreviewed notification/card review queues"
	@echo "  make event-alpha-notification-deliveries-report PROFILE=notify_no_key  Print the notification delivery ledger"
...
# and idempotent delivery ledger (both default-on), use real wall-clock time (no
# fixed research clock), fail soft on provider errors (exit 0 on partial failure),
# and never paper/live trade or write normal RSI rows.
