...
PYTEST_FILE_TIMING_MD ?= .pytest_cache/test_file_timing_report.md
PYTEST_FILE_TIMING_ARGS = -p crypto_rsi_scanner.pytest_file_timing --slowest-test-files=$(PYTEST_DURATIONS) --test-file-timing-json=$(PYTEST_FILE_TIMING_JSON) --test-file-timing-md=$(PYTEST_FILE_TIMING_MD)
ARTIFACT_HEAVY_TEST_MAX_SECONDS ?= 5.0
PYTEST_WORKERS ?= auto
...
EVENT_ALPHA_DEX_GECKOTERMINAL_PATH ?= fixtures/event_dex_onchain/geckoterminal_pools.json
EVENT_ALPHA_DEX_COINGECKO_PATH ?= fixtures/event_dex_onchain/coingecko_dex_pools.json
EVENT_ALPHA_PROTOCOL_DEFILLAMA_PATH ?= fixtures/event_dex_onchain/defillama_protocol_fundamentals.json
...
RADAR_DASHBOARD_HOST ?= 127.0.0.1
...
RADAR_REVIEW_INSPECTION_OUTPUT ?= card
...
BURN_IN_DOCTOR_MODE ?= scoped_burn_in
BURN_IN_DOCTOR_TIMEOUT_SECONDS ?= 120
BURN_IN_DOCTOR_OPTIONAL_ARG = $(if $(filter 0 false no,$(DOCTOR_REQUIRED)),--doctor-optional,)
BURN_IN_DOCTOR_ARGS = --doctor-mode $(BURN_IN_DOCTOR_MODE) --doctor-timeout-seconds $(BURN_IN_DOCTOR_TIMEOUT_SECONDS) $(BURN_IN_DOCTOR_OPTIONAL_ARG)
...
BURN_IN_INCLUDE_NOTIFICATION_ARG = $(if $(filter 1 true yes,$(INCLUDE_NOTIFICATION_REHEARSALS)),--include-notification-rehearsals,)
BURN_IN_INCLUDE_NO_KEY_ARG = $(if $(filter 1 true yes,$(INCLUDE_NO_KEY_NAMESPACES)),--include-no-key-namespaces,)
BURN_IN_INCLUDE_PROVIDER_ARG = $(if $(filter 1 true yes,$(INCLUDE_PROVIDER_REHEARSALS)),--include-provider-rehearsals,)
BURN_IN_INCLUDE_FIXTURE_ARG = $(if $(filter 1 true yes,$(INCLUDE_FIXTURE_NAMESPACES)),--include-fixture-namespaces,)
BURN_IN_INCLUDE_STALE_ARG = $(if $(filter 1 true yes,$(INCLUDE_STALE_NAMESPACES)),--include-stale-namespaces,)
BURN_IN_INCLUDE_LIVE_REHEARSAL_WITHOUT_RUN_ARG = $(if $(filter 1 true yes,$(INCLUDE_LIVE_REHEARSALS_WITHOUT_BURN_IN_RUN)),--include-live-rehearsals-without-burn-in-run,)
BURN_IN_COUNT_EXPLICIT_ARG = $(if $(filter 1 true yes,$(COUNT_EXPLICIT_NAMESPACE_FOR_BURN_IN)),--count-explicit-namespace-for-burn-in,)
BURN_IN_POLICY_FLAGS = $(BURN_IN_INCLUDE_NOTIFICATION_ARG) $(BURN_IN_INCLUDE_NO_KEY_ARG) $(BURN_IN_INCLUDE_PROVIDER_ARG) $(BURN_IN_INCLUDE_FIXTURE_ARG) $(BURN_IN_INCLUDE_STALE_ARG) $(BURN_IN_INCLUDE_LIVE_REHEARSAL_WITHOUT_RUN_ARG)
PROVIDER_KEY ?=
PROVIDER_SERVICE ?=
PROVIDER_ROLE ?=
PROVIDER_ALL ?= 0
...
.PHONY: verify-fast test-full test-rsi test-cli test-pytest test-pytest-safe test-pytest-timed test-pytest-durations test-pytest-parallel test-event-alpha architecture-baseline architecture-api-inventory architecture-class-ownership-report architecture-size-baseline-update architecture-size-report architecture-size-gates architecture-transitional-file-check architecture-terminology-check architecture-naming-check architecture-final-report architecture-completion-map architecture-cleanliness-check event-alpha-radar-north-star event-alpha-namespace-lifecycle-report event-alpha-list-active-namespaces event-alpha-archive-stale-namespaces radar-dashboard radar-dashboard-validate radar-dashboard-readiness radar-dashboard-publish radar-dashboard-authority-status radar-dashboard-invalidate radar-dashboard-render-fixture radar-dashboard-smoke radar-dashboard-ux-smoke radar-dashboard-phone-readiness radar-dashboard-phone-status radar-dashboard-phone-enable radar-dashboard-phone-disable radar-dashboard-public-readiness radar-dashboard-public-status radar-dashboard-public-enable radar-dashboard-public-disable radar-dashboard-public-guard radar-calendar-preview radar-calendar-official-readiness radar-calendar-official-acquire radar-calendar-official-import-local radar-unlock-tokenomist-v5-smoke radar-fundamentals-defillama-smoke radar-fundamentals-defillama-mapping-smoke radar-fundamentals-defillama-mapping-review radar-dex-onchain-evm-v2-smoke radar-dex-onchain-evm-v2-validate-local radar-dex-onchain-evm-v2-import-local radar-dex-onchain-evm-v2-status radar-execution-quality-readiness radar-execution-quality-readiness-full radar-execution-quality-readiness-json radar-execution-quality-bybit-smoke radar-derivatives-bybit-smoke radar-derivatives-bybit-liquidation-smoke radar-derivatives-bybit-liquidation-capture-smoke radar-derivatives-bybit-liquidation-validate-local radar-derivatives-bybit-liquidation-import-local radar-derivatives-bybit-liquidation-status radar-derivatives-bybit-readiness radar-derivatives-bybit-collect radar-derivatives-bybit-capture radar-derivatives-bybit-status radar-intraday-bybit-smoke radar-intraday-bybit-readiness radar-intraday-bybit-collect radar-intraday-bybit-capture radar-intraday-bybit-status radar-execution-quality-bybit-readiness radar-execution-quality-bybit-collect radar-execution-quality-bybit-capture radar-execution-quality-bybit-status radar-market-no-send-readiness radar-market-no-send radar-market-no-send-smoke radar-market-campaign-report radar-outcome-price-recovery-readiness radar-outcome-price-recovery-collect radar-outcome-price-recovery-capture radar-outcome-price-recovery-status radar-outcome-price-recovery-apply radar-outcome-price-recovery-application-status radar-review-timing-status radar-review-timing-queue radar-review-timing-inspect radar-review-timing-view radar-review-timing-complete radar-daily-ops-readiness radar-daily-ops-status radar-daily-ops-dry-run radar-daily-ops-cycle radar-daily-ops-reconcile-publication radar-daily-ops-install radar-daily-ops-uninstall
.PHONY: test-artifact-heavy-extracted-checkout
...
help:
	...
	@echo "  make test-rsi  Run pytest over split RSI/backtest/paper tests"
	...
	@echo "  make test-artifact-heavy-extracted-checkout  Run the bounded cumulative-artifact performance guard"
	...
	@echo "  make radar-dashboard-readiness ARTIFACT_NAMESPACE=...  Validate one dashboard generation, read-only"
	@echo "  CONFIRM=1 make radar-dashboard-publish ARTIFACT_NAMESPACE=...  Publish one receipt-backed operational generation"
	@echo "  make radar-dashboard-authority-status  Inspect persisted dashboard authority, read-only"
	@echo "  CONFIRM=1 make radar-dashboard-invalidate ARTIFACT_NAMESPACE=...  Remove only the exact named pointer"
	@echo "  make radar-dashboard-smoke        Render every dashboard page from fixtures, no server/writes"
	@echo "  make radar-dashboard-ux-smoke     Validate primary dashboard semantics and responsive contracts offline"
	@echo "  make radar-dashboard-phone-readiness  Check private tailnet phone access without changing it"
	@echo "  make radar-dashboard-phone-status     Show sanitized dashboard/Tailscale Serve status"
	@echo "  CONFIRM=1 make radar-dashboard-phone-enable  Enable private tailnet HTTPS phone access"
	@echo "  CONFIRM=1 make radar-dashboard-phone-disable Disable only the owned private HTTPS route"
	@echo "  make radar-dashboard-public-readiness Check temporary public-link readiness without changing it"
	@echo "  make radar-dashboard-public-status    Show sanitized Cloudflare Quick Tunnel status"
	...
	@echo "  make radar-review-timing-inspect RADAR_REVIEW_NAMESPACE=... RADAR_REVIEW_IDEA_ID=...  Verify and render the exact card; no timing event/write"
	...
	@echo "  make radar-dex-onchain-evm-v2-status EVM_POOL_CAPTURE_NAMESPACE=...  Revalidate one exact immutable capture"
	@echo "  make radar-execution-quality-readiness  Concise selected Bybit-perpetual readiness; no call/write"
	@echo "  make radar-execution-quality-readiness-full  Full static venue/cost catalog; no call/write"
	@echo "  make radar-execution-quality-bybit-smoke  Normalize Bybit public fixtures; no call/write"
	@echo "  make radar-derivatives-bybit-smoke  Normalize venue-native Bybit funding/OI/basis/positioning fixtures; no call/write"
	@echo "  make radar-derivatives-bybit-liquidation-smoke  Normalize exact Bybit public liquidation messages; no socket/call/write"
	@echo "  make radar-derivatives-bybit-liquidation-capture-smoke  Prove disposable transcript capture; no socket/call retained"
	...
	@echo "  make event-alpha-dex-onchain-readiness-smoke  Write fixture/no-call DEX/on-chain readiness artifacts"
	...
	@echo "  make event-alpha-namespace-lifecycle-report  Write Event Alpha namespace inventory/report"
	@echo "  make event-alpha-list-active-namespaces  List active Event Alpha namespaces"
	@echo "  make event-alpha-archive-stale-namespaces  Dry-run archive plan for stale Event Alpha namespaces"
	...
	@echo "  make architecture-size-baseline-update  Refresh optional static size trend baseline"
	@echo "  make architecture-size-report  Write advisory static size inventory"
	@echo "  make architecture-size-gates  Legacy alias for advisory size inventory"
	...
	@echo "  make architecture-final-report  Write architecture organization/shim report with advisory size metrics"
	@echo "  make architecture-completion-map  Write final architecture completion and release-candidate reports"
	@echo "  make architecture-cleanliness-check  Run naming/import/organization gates plus advisory size inventory"
	@echo "  make event-alpha-radar-north-star  Write Event Alpha Radar North Star contract"
	@echo "  make event-alpha-burn-in-contract  Write separate Event Alpha burn-in contract"
	...
	@echo "  make event-alpha-daily-live-no-send-burn-in-candidate-mode-smoke  Run deterministic mocked candidate-mode smoke"
	@echo "  make event-alpha-daily-live-no-send-burn-in-candidate-mode-smoke-doctor  Strict doctor check for candidate-mode smoke artifacts"
	@echo "  make event-alpha-daily-burn-in-readiness  Report candidate-mode provider readiness without live calls"
	@echo "  make event-alpha-daily-review-inbox  Write the compact daily review inbox"
	@echo "  make event-alpha-feedback-progress  Summarize burn-in label coverage"
	@echo "  make event-alpha-burn-in-weekly-measurement  Write weekly burn-in measurement dashboard"
	@echo "  make event-alpha-source-yield-report  Write provider/source-pack yield recommendations"
	@echo "  make conviction-priors-shadow-report  Write shadow-only conviction prior diagnostics"
	@echo "  make paper-risk-research  Write paper-risk scenario research"
	...
	@echo "  make event-feedback-source-noise FEEDBACK_TARGET=...  Mark quick source-noise feedback"
...
test:
	...
	RSI_EVENT_DISCOVERY_CRYPTOPANIC_LIVE=0 \
	...
	RSI_EVENT_ALPHA_BYBIT_ANNOUNCEMENTS_ALLOW_LIVE_PREFLIGHT=0 \
	RSI_EVENT_ALPHA_COINALYZE_ALLOW_LIVE_PREFLIGHT=0 \
	RSI_EVENT_ALPHA_DEX_ONCHAIN_ALLOW_LIVE_PREFLIGHT=0 \
	RSI_EVENT_ALPHA_UNLOCK_CALENDAR_ALLOW_LIVE_PREFLIGHT=0 \
	RSI_DECISION_RADAR_BYBIT_EXECUTION_QUALITY_LIVE=0 \
	RSI_DECISION_RADAR_BYBIT_INTRADAY_LIVE=0 \
	RSI_DECISION_RADAR_BYBIT_DERIVATIVES_LIVE=0 \
	RSI_DECISION_RADAR_MACRO_CALENDAR_LIVE=0 \
...
# fails verify instead of silently skipping the real test suite. This is the
# gate that runs the CLI ops smokes and the relative-import integrity check
# (regression: 2026-07-03 refactor broke --status/--backup-db/--maintenance
# while verify stayed green).
test-full:
	@if ! $(PYTHON) -c "import pytest" >/dev/null 2>&1; then \
		echo "pytest is required for make verify; run: $(PYTHON) -m pip install -r requirements.txt"; \
		exit 1; \
	fi
	$(PYTEST_SAFE_ENV) $(PYTHON) -m pytest $(PYTEST_PATHS) -q $(PYTEST_FILE_TIMING_ARGS)

test-rsi:
	$(PYTEST_SAFE_ENV) $(PYTHON) -m pytest tests/rsi

test-cli:
	$(PYTEST_SAFE_ENV) $(PYTHON) -m pytest tests/cli

test-event-alpha:
	$(PYTEST_SAFE_ENV) $(PYTHON) -m pytest tests/event_alpha

test-pytest-safe:
	@if $(PYTHON) -c "import pytest" >/dev/null 2>&1; then \
		$(PYTEST_SAFE_ENV) $(PYTHON) -m pytest $(PYTEST_PATHS); \
	else \
...
test-pytest-durations:
	@if ! $(PYTHON) -c "import pytest" >/dev/null 2>&1; then \
		echo "pytest is required for duration reporting; run: $(PYTHON) -m pip install -r requirements.txt"; \
		exit 1; \
	fi
	$(PYTEST_SAFE_ENV) $(PYTHON) -m pytest $(PYTEST_PATHS) -q --durations=$(PYTEST_DURATIONS)

test-artifact-heavy-extracted-checkout:
	@if ! $(PYTHON) -c "import pytest" >/dev/null 2>&1; then \
		echo "pytest is required for the artifact-heavy performance guard; run: $(PYTHON) -m pip install -r requirements.txt"; \
		exit 1; \
	fi
	RSI_ARTIFACT_HEAVY_TEST_MAX_SECONDS=$(ARTIFACT_HEAVY_TEST_MAX_SECONDS) $(PYTEST_SAFE_ENV) $(PYTHON) -m pytest tests/cli/test_project_health_performance.py -q -p crypto_rsi_scanner.pytest_file_timing --slowest-test-files=10 --test-file-timing-json=.pytest_cache/artifact_heavy_extracted_checkout_timing.json --test-file-timing-md=.pytest_cache/artifact_heavy_extracted_checkout_timing.md

test-pytest-parallel:
	@if ! $(PYTHON) -c "import pytest" >/dev/null 2>&1; then \
		echo "pytest is not installed; install pytest or use 'make test' for the standalone runner."; \
	elif $(PYTHON) -c "import xdist" >/dev/null 2>&1; then \
		$(PYTEST_SAFE_ENV) $(PYTHON) -m pytest -p xdist.plugin $(PYTEST_PATHS) -q -n $(PYTEST_WORKERS) --dist=$(PYTEST_XDIST_MODE); \
	else \
		echo "pytest-xdist is not installed; skipping parallel pytest run. Install pytest-xdist or use 'make test-pytest PYTHON=$(PYTHON)' for the serial suite."; \
	fi
...
architecture-size-gates: architecture-size-report
...
event-alpha-radar-north-star:
	$(PYTHON) -m crypto_rsi_scanner.project_health.radar_north_star
...
event-alpha-integrated-radar-smoke:
	...
	test -s $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_market_state_snapshots.jsonl
	test -s $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_market_anomalies.jsonl
	test -s $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_market_anomaly_catalyst_search_queue.jsonl
	test -s $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_dex_onchain_readiness.json
	test -s $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_dex_pool_state.jsonl
	test -s $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_dex_pool_anomalies.jsonl
	test -s $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_protocol_fundamentals.jsonl
	test -s $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_exchange_announcements.jsonl
	test -s $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_official_exchange_events.jsonl
	test -s $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_official_listing_candidates.jsonl
	test -s $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_scheduled_catalysts.jsonl
	test -s $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_unlock_candidates.jsonl
	test -s $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_derivatives_state.jsonl
	test -s $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_derivatives_crowding_candidates.jsonl
	test -s $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_fade_short_review_candidates.jsonl
	test -s $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_integrated_radar_candidates.jsonl
	test -s $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_integrated_radar_outcomes.jsonl
	test -s $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_integrated_radar_report.md
	test -s $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_integrated_radar_input_manifest.json
	test -s $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_alpha_source_coverage.json
	test -s $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_core_opportunities.jsonl
	test -s $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_integrated_radar_notification_deliveries.jsonl
	test -s $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_integrated_radar_notification_preview.md
	test -s $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_decision_v2_notification_preview.md
	test -s $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_alpha_daily_brief.md
	test -s $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_unified_calendar_events.jsonl
	test -s $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_unified_calendar_preview.md
	grep -q "EARLY_LONG_RESEARCH" $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_integrated_radar_candidates.jsonl
	grep -q "CONFIRMED_LONG_RESEARCH" $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_integrated_radar_candidates.jsonl
	grep -q "FADE_SHORT_REVIEW" $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_integrated_radar_candidates.jsonl
	grep -q "RISK_ONLY" $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_integrated_radar_candidates.jsonl
	grep -q "UNCONFIRMED_RESEARCH" $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_integrated_radar_candidates.jsonl
	grep -q "DIAGNOSTIC" $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_integrated_radar_candidates.jsonl
	grep -q "DEX / On-Chain Liquidity" $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_alpha_daily_brief.md
	grep -q "Protocol Fundamentals" $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_alpha_daily_brief.md
	grep -q "Research-only" $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_integrated_radar_notification_preview.md
	grep -q "Crypto Radar Decision v2 Preview" $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_decision_v2_notification_preview.md
	! grep -q "Lane: Early Long Research" $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_decision_v2_notification_preview.md
	grep -q "would_send_but_guard_disabled" $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_integrated_radar_notification_deliveries.jsonl
	env RSI_EVENT_ALPHA_ARTIFACT_BASE_DIR=$(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
	...
	$(MAKE) radar-dashboard-render-fixture PYTHON=$(PYTHON) RADAR_DASHBOARD_FIXTURE_BASE=$(EVENT_ALPHA_ARTIFACT_BASE_DIR) RADAR_DASHBOARD_FIXTURE_NAMESPACE=$(PROFILE)
	@echo "Integrated-radar smoke artifacts under $(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/."
...
event-alpha-market-anomaly-smoke:
	...
	RSI_EVENT_ALPHA_ARTIFACT_NAMESPACE=$(PROFILE) \
	RSI_EVENT_ALERTS_ENABLED=0 \
	RSI_EVENT_ALPHA_DEX_GECKOTERMINAL_PATH=$(EVENT_ALPHA_DEX_GECKOTERMINAL_PATH) \
	RSI_EVENT_ALPHA_DEX_COINGECKO_PATH=$(EVENT_ALPHA_DEX_COINGECKO_PATH) \
	RSI_EVENT_ALPHA_PROTOCOL_DEFILLAMA_PATH=$(EVENT_ALPHA_PROTOCOL_DEFILLAMA_PATH) \
	$(PYTHON) main.py --event-alpha-dex-onchain-readiness-smoke --event-alpha-profile fixture --event-alpha-artifact-namespace $(PROFILE) --event-alpha-include-test-artifacts
	...
	test -s event_fade_cache/$(PROFILE)/event_dex_onchain_readiness.json
	test -s event_fade_cache/$(PROFILE)/event_dex_pool_state.jsonl
	test -s event_fade_cache/$(PROFILE)/event_dex_pool_anomalies.jsonl
	test -s event_fade_cache/$(PROFILE)/event_protocol_fundamentals.jsonl
...
radar-dashboard-validate:
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.dashboard \
		--artifact-base $(EVENT_ALPHA_ARTIFACT_BASE_DIR) $(RADAR_DASHBOARD_NAMESPACE_ARG) \
		--validate

radar-dashboard-readiness: radar-dashboard-validate

radar-dashboard-publish:
	@test "$(CONFIRM)" = "1" || { echo "Refusing dashboard publication without CONFIRM=1" >&2; exit 2; }
	@test -n "$(RADAR_DASHBOARD_EXPLICIT_NAMESPACE)" || { echo "ARTIFACT_NAMESPACE is required" >&2; exit 2; }
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.dashboard \
		--artifact-base $(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
		--namespace $(RADAR_DASHBOARD_EXPLICIT_NAMESPACE) \
		--publish --confirm-publish

radar-dashboard-authority-status:
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.dashboard \
		--artifact-base $(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
		--authority-status

radar-dashboard-invalidate:
	@test "$(CONFIRM)" = "1" || { echo "Refusing dashboard invalidation without CONFIRM=1" >&2; exit 2; }
	@test -n "$(RADAR_DASHBOARD_EXPLICIT_NAMESPACE)" || { echo "ARTIFACT_NAMESPACE is required" >&2; exit 2; }
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.dashboard \
		--artifact-base $(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
		--namespace $(RADAR_DASHBOARD_EXPLICIT_NAMESPACE) \
		--invalidate-authority --confirm-invalidate

radar-dashboard-render-fixture:
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.dashboard \
		--artifact-base $(RADAR_DASHBOARD_FIXTURE_BASE) \
		--namespace $(RADAR_DASHBOARD_FIXTURE_NAMESPACE) \
		--smoke

radar-dashboard-smoke: radar-dashboard-render-fixture

radar-dashboard-ux-smoke:
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.dashboard.ux_smoke \
		--artifact-base $(RADAR_DASHBOARD_FIXTURE_BASE) \
		--namespace $(RADAR_DASHBOARD_FIXTURE_NAMESPACE)

radar-dashboard-phone-readiness:
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.dashboard.phone_access readiness

radar-dashboard-phone-status:
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.dashboard.phone_access status

radar-dashboard-phone-enable:
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.dashboard.phone_access enable \
		$(if $(filter 1,$(CONFIRM)),--confirm,)

radar-dashboard-phone-disable:
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.dashboard.phone_access disable \
		$(if $(filter 1,$(CONFIRM)),--confirm,)

radar-dashboard-public-readiness:
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.dashboard.public_access readiness

radar-dashboard-public-status:
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.dashboard.public_access status

radar-dashboard-public-enable:
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.dashboard.public_access enable \
		$(if $(filter 1,$(CONFIRM)),--confirm,)

radar-dashboard-public-disable:
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.dashboard.public_access disable \
		$(if $(filter 1,$(CONFIRM)),--confirm,)

radar-dashboard-public-guard:
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.dashboard.public_access guard \
		$(if $(filter 1,$(CONFIRM)),--confirm,)
...
radar-market-no-send-readiness:
	...
	RSI_EVENT_ALPHA_ARTIFACT_BASE_DIR=$(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.market_no_send readiness \
		--artifact-base $(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
...
radar-daily-ops-dry-run:
	env RSI_EVENT_ALERTS_ENABLED=0 \
	RSI_EVENT_ALPHA_RUN_MODE=operational \
	RSI_EVENT_ALPHA_TARGETED_MARKET_REFRESH_ENABLED=0 \
	RSI_EVENT_ALPHA_ARTIFACT_BASE_DIR=$(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.daily_operations cycle \
		--dry-run --artifact-base $(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
		--top-n $(RADAR_MARKET_NO_SEND_TOP_N) $(RADAR_MARKET_NO_SEND_FETCH_ARG) \
		--interval-seconds $(RADAR_DAILY_OPS_INTERVAL_SECONDS)
...
radar-daily-ops-install:
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.daily_operations install \
		--artifact-base $(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
		--top-n $(RADAR_MARKET_NO_SEND_TOP_N) $(RADAR_MARKET_NO_SEND_FETCH_ARG) \
		--interval-seconds $(RADAR_DAILY_OPS_INTERVAL_SECONDS) \
		$(if $(filter 1,$(CONFIRM)),--confirm,)
...
radar-daily-ops-uninstall:
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.daily_operations uninstall \
		--artifact-base $(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
		--top-n $(RADAR_MARKET_NO_SEND_TOP_N) $(RADAR_MARKET_NO_SEND_FETCH_ARG) \
		--interval-seconds $(RADAR_DAILY_OPS_INTERVAL_SECONDS) \
		$(if $(filter 1,$(CONFIRM)),--confirm,)
...
radar-market-campaign-report:
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.market_no_send campaign-report \
		...
		$(RADAR_MARKET_CAMPAIGN_EVALUATED_AT_ARG)
...
radar-outcome-price-recovery-capture:
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.outcome_price_recovery_capture capture \
		--artifact-base $(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
		$(if $(filter 1,$(CONFIRM)),--confirm,)

radar-outcome-price-recovery-status:
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.outcome_price_recovery_capture status \
		--artifact-base $(EVENT_ALPHA_ARTIFACT_BASE_DIR)

radar-outcome-price-recovery-apply:
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.outcome_price_recovery_application apply \
		--artifact-base $(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
		$(if $(filter 1,$(CONFIRM)),--confirm,)

radar-outcome-price-recovery-application-status:
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.outcome_price_recovery_application status \
		--artifact-base $(EVENT_ALPHA_ARTIFACT_BASE_DIR)
...
radar-review-timing-inspect:
	...
	@test -n "$(strip $(RADAR_REVIEW_IDEA_ID))" || { echo "RADAR_REVIEW_IDEA_ID is required" >&2; exit 2; }
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.decision_review_timing_cli \
		--artifact-base $(EVENT_ALPHA_ARTIFACT_BASE_DIR) inspect \
		--namespace "$(RADAR_REVIEW_NAMESPACE)" \
		--idea-id "$(RADAR_REVIEW_IDEA_ID)" \
		$(RADAR_REVIEW_EVALUATED_AT_ARG) --output $(RADAR_REVIEW_INSPECTION_OUTPUT)

radar-review-timing-view:
	@test -n "$(strip $(RADAR_REVIEW_NAMESPACE))" || { echo "RADAR_REVIEW_NAMESPACE is required" >&2; exit 2; }
	@test -n "$(strip $(RADAR_REVIEW_IDEA_ID))" || { echo "RADAR_REVIEW_IDEA_ID is required" >&2; exit 2; }
	@test -n "$(strip $(RADAR_REVIEWER_ALIAS))" || { echo "RADAR_REVIEWER_ALIAS is required" >&2; exit 2; }
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.decision_review_timing_cli \
		--artifact-base $(EVENT_ALPHA_ARTIFACT_BASE_DIR) view \
		--namespace "$(RADAR_REVIEW_NAMESPACE)" \
		--idea-id "$(RADAR_REVIEW_IDEA_ID)" \
...
radar-review-timing-complete:
	...
	@test -n "$(strip $(RADAR_REVIEWER_ALIAS))" || { echo "RADAR_REVIEWER_ALIAS is required" >&2; exit 2; }
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.decision_review_timing_cli \
		--artifact-base $(EVENT_ALPHA_ARTIFACT_BASE_DIR) complete \
		--namespace "$(RADAR_REVIEW_NAMESPACE)" \
		--idea-id "$(RADAR_REVIEW_IDEA_ID)" \
		--reviewer-alias "$(RADAR_REVIEWER_ALIAS)" \
		$(if $(filter 1,$(CONFIRM)),--confirm,)
...
radar-market-no-send-smoke:
	...
	RSI_EVENT_ALPHA_TARGETED_MARKET_REFRESH_ENABLED=0 \
	RSI_EVENT_ALPHA_ARTIFACT_BASE_DIR=$(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.market_no_send smoke \
		--artifact-base $(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
		--namespace $(RADAR_MARKET_NO_SEND_SMOKE_NAMESPACE) \
	...
	RSI_EVENT_ALERTS_ENABLED=0 \
	RSI_EVENT_ALPHA_ARTIFACT_BASE_DIR=$(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
	sh -c 'cd "$(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(RADAR_MARKET_NO_SEND_SMOKE_NAMESPACE)" && exec "$(RADAR_MARKET_NO_SEND_PYTHON)" "$(RADAR_MARKET_NO_SEND_MAIN)" --event-alpha-artifact-doctor \
		--event-alpha-profile fixture \
		--event-alpha-artifact-namespace $(RADAR_MARKET_NO_SEND_SMOKE_NAMESPACE) \
	...
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.dashboard \
		...
		--namespace $(RADAR_MARKET_NO_SEND_SMOKE_NAMESPACE) \
		--smoke
	@echo "Mock generation was rendered and verified without invoking publication."

radar-calendar-preview:
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.radar.calendar
...
radar-announcements-bitget-readiness:
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.bitget_announcements_readiness
...
radar-fundamentals-defillama-smoke:
	$(PYTHON) -m crypto_rsi_scanner.event_providers.defillama_fundamentals \
		fixtures/event_dex_onchain/defillama_fundamentals_official_capture.json

radar-fundamentals-defillama-mapping-smoke:
	$(PYTHON) -m crypto_rsi_scanner.event_providers.defillama_mapping_registry \
		fixtures/event_dex_onchain/defillama_mapping_market_rows.json \
		--registry fixtures/event_dex_onchain/defillama_mapping_registry.json \
...
radar-dex-onchain-evm-v2-smoke:
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.evm_v2_pool_snapshot \
		--input fixtures/event_dex_onchain/evm_v2_pool_rpc_bundle.json \
		--capture-mode fixture

radar-dex-onchain-evm-v2-validate-local:
	@test -n "$(strip $(EVM_POOL_RPC_BUNDLE))" || { echo "EVM_POOL_RPC_BUNDLE is required" >&2; exit 2; }
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.evm_v2_pool_capture validate-local \
		--input "$(EVM_POOL_RPC_BUNDLE)"

radar-dex-onchain-evm-v2-import-local:
	@test -n "$(strip $(EVM_POOL_RPC_BUNDLE))" || { echo "EVM_POOL_RPC_BUNDLE is required" >&2; exit 2; }
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.evm_v2_pool_capture import-local \
		--artifact-base $(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
		--input "$(EVM_POOL_RPC_BUNDLE)" \
		$(if $(filter 1,$(CONFIRM)),--confirm,)

radar-dex-onchain-evm-v2-status:
	@test -n "$(strip $(EVM_POOL_CAPTURE_NAMESPACE))" || { echo "EVM_POOL_CAPTURE_NAMESPACE is required" >&2; exit 2; }
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.evm_v2_pool_capture status \
		--artifact-base $(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
		--namespace "$(EVM_POOL_CAPTURE_NAMESPACE)"

radar-execution-quality-readiness:
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.execution_quality_readiness --summary

radar-execution-quality-readiness-full:
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.execution_quality_readiness

radar-execution-quality-readiness-json:
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.execution_quality_readiness --json

radar-execution-quality-bybit-smoke:
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.bybit_execution_quality --fixture-dir fixtures/bybit_execution_quality

radar-derivatives-bybit-smoke:
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.bybit_derivatives_context --fixture-dir fixtures/bybit_derivatives_context

radar-derivatives-bybit-liquidation-smoke:
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.bybit_liquidation_stream \
		--fixture-dir fixtures/bybit_liquidation_stream \
		--execution-fixture-dir fixtures/bybit_execution_quality

radar-derivatives-bybit-liquidation-capture-smoke:
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.bybit_liquidation_capture capture-smoke

radar-derivatives-bybit-liquidation-validate-local:
	@test -n "$(strip $(BYBIT_LIQUIDATION_TRANSCRIPT))" || { echo "BYBIT_LIQUIDATION_TRANSCRIPT is required" >&2; exit 2; }
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.bybit_liquidation_capture validate-local \
		--input "$(BYBIT_LIQUIDATION_TRANSCRIPT)"

radar-derivatives-bybit-liquidation-import-local:
	@test -n "$(strip $(BYBIT_LIQUIDATION_TRANSCRIPT))" || { echo "BYBIT_LIQUIDATION_TRANSCRIPT is required" >&2; exit 2; }
	@test "$(CONFIRM)" = "1" || { echo "CONFIRM=1 is required to seal an operator transcript" >&2; exit 2; }
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.bybit_liquidation_capture import-local \
		--artifact-base "$(EVENT_ALPHA_ARTIFACT_BASE_DIR)" \
		--input "$(BYBIT_LIQUIDATION_TRANSCRIPT)" \
		--confirm

radar-derivatives-bybit-liquidation-status:
	@test -n "$(strip $(BYBIT_LIQUIDATION_CAPTURE_NAMESPACE))" || { echo "BYBIT_LIQUIDATION_CAPTURE_NAMESPACE is required" >&2; exit 2; }
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.bybit_liquidation_capture status \
		--artifact-base "$(EVENT_ALPHA_ARTIFACT_BASE_DIR)" \
		--namespace "$(BYBIT_LIQUIDATION_CAPTURE_NAMESPACE)"
...
radar-derivatives-bybit-collect:
	env RSI_EVENT_ALERTS_ENABLED=0 \
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.bybit_derivatives_context_live collect \
		--artifact-base $(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
		$(if $(filter 1,$(CONFIRM)),--confirm,)

radar-derivatives-bybit-capture:
	env RSI_EVENT_ALERTS_ENABLED=0 \
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.bybit_derivatives_context_live capture \
		--artifact-base $(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
		$(if $(filter 1,$(CONFIRM)),--confirm,)

radar-derivatives-bybit-status:
	env RSI_EVENT_ALERTS_ENABLED=0 \
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.bybit_derivatives_context_live status \
		--artifact-base $(EVENT_ALPHA_ARTIFACT_BASE_DIR)

radar-intraday-bybit-smoke:
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.bybit_intraday --fixture-dir fixtures/bybit_intraday
...
radar-intraday-bybit-collect:
	env RSI_EVENT_ALERTS_ENABLED=0 \
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.bybit_intraday_live collect \
		--artifact-base $(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
		$(if $(filter 1,$(CONFIRM)),--confirm,)

radar-intraday-bybit-capture:
	env RSI_EVENT_ALERTS_ENABLED=0 \
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.bybit_intraday_live capture \
		--artifact-base $(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
		$(if $(filter 1,$(CONFIRM)),--confirm,)

radar-intraday-bybit-status:
	env RSI_EVENT_ALERTS_ENABLED=0 \
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.bybit_intraday_live status \
		--artifact-base $(EVENT_ALPHA_ARTIFACT_BASE_DIR)
...
radar-execution-quality-bybit-collect:
	env RSI_EVENT_ALERTS_ENABLED=0 \
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.bybit_execution_quality_live collect \
		--artifact-base $(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
		$(if $(filter 1,$(CONFIRM)),--confirm,)

radar-execution-quality-bybit-capture:
	env RSI_EVENT_ALERTS_ENABLED=0 \
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.bybit_execution_quality_live capture \
		--artifact-base $(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
		$(if $(filter 1,$(CONFIRM)),--confirm,)

radar-execution-quality-bybit-status:
	env RSI_EVENT_ALERTS_ENABLED=0 \
...
event-alpha-dex-onchain-readiness:
	...
	RSI_EVENT_ALPHA_DEX_COINGECKO_PATH=$(EVENT_ALPHA_DEX_COINGECKO_PATH) \
	RSI_EVENT_ALPHA_PROTOCOL_DEFILLAMA_PATH=$(EVENT_ALPHA_PROTOCOL_DEFILLAMA_PATH) \
	$(PYTHON) main.py --event-alpha-dex-onchain-readiness --event-alpha-profile $(EVENT_ALPHA_RUNTIME_PROFILE) --event-alpha-artifact-namespace $(ARTIFACT_NAMESPACE)
	test -s "$(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(ARTIFACT_NAMESPACE)/event_dex_onchain_readiness.json"
	test -s "$(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(ARTIFACT_NAMESPACE)/event_dex_onchain_readiness.md"
	test -s "$(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(ARTIFACT_NAMESPACE)/event_dex_pool_state.jsonl"
	test -s "$(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(ARTIFACT_NAMESPACE)/event_dex_pool_anomalies.jsonl"
	test -s "$(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(ARTIFACT_NAMESPACE)/event_protocol_fundamentals.jsonl"

event-alpha-dex-onchain-readiness-smoke: PROFILE = dex_onchain_readiness_smoke
event-alpha-dex-onchain-readiness-smoke:
	...
	$(PYTHON) main.py --event-alpha-dex-onchain-readiness-smoke --event-alpha-profile fixture --event-alpha-artifact-namespace $(PROFILE) --event-alpha-include-test-artifacts
	test -s "$(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_dex_onchain_readiness.json"
	test -s "$(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_dex_onchain_readiness.md"
	test -s "$(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_dex_pool_state.jsonl"
	test -s "$(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_dex_pool_anomalies.jsonl"
	test -s "$(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_protocol_fundamentals.jsonl"
	grep -q '"live_call_allowed": false' "$(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_dex_onchain_readiness.json"
	grep -q "dex_liquidity_expansion" "$(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_dex_pool_anomalies.jsonl"
	grep -q "suspicious_low_liquidity_pump" "$(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_dex_pool_anomalies.jsonl"
	grep -q "protocol_revenue_tvl_growth" "$(EVENT_ALPHA_ARTIFACT_BASE_DIR)/$(PROFILE)/event_protocol_fundamentals.jsonl"
	$(PYTHON) main.py --event-alpha-source-coverage-report --event-alpha-profile fixture --event-alpha-artifact-namespace $(PROFILE) --event-alpha-include-test-artifacts
	$(PYTHON) main.py --event-alpha-artifact-doctor --event-alpha-profile fixture --event-alpha-artifact-namespace $(PROFILE) --event-alpha-include-test-artifacts
...
event-alpha-namespace-lifecycle-report:
	RSI_EVENT_ALPHA_ARTIFACT_BASE_DIR=$(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
	RSI_EVENT_ALERTS_ENABLED=0 \
	$(PYTHON) main.py --event-alpha-namespace-lifecycle-report

event-alpha-list-active-namespaces:
	RSI_EVENT_ALPHA_ARTIFACT_BASE_DIR=$(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
	RSI_EVENT_ALERTS_ENABLED=0 \
	$(PYTHON) main.py --event-alpha-list-active-namespaces
...
event-alpha-daily-live-no-send-burn-in:
	RSI_EVENT_ALERTS_ENABLED=0 \
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.daily_burn_in --profile live_burn_in_no_send --python $(PYTHON) $(BURN_IN_NAMESPACE_ARG) $(BURN_IN_CANDIDATE_MODE_ARG) $(BURN_IN_DOCTOR_ARGS)

event-alpha-daily-live-no-send-burn-in-plan:
	RSI_EVENT_ALERTS_ENABLED=0 \
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.daily_burn_in --profile live_burn_in_no_send --python $(PYTHON) $(BURN_IN_NAMESPACE_ARG) $(BURN_IN_CANDIDATE_MODE_ARG) $(BURN_IN_DOCTOR_ARGS) --dry-run-plan

event-alpha-daily-live-no-send-burn-in-smoke:
	RSI_EVENT_ALERTS_ENABLED=0 \
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.daily_burn_in --profile fixture --artifact-namespace daily_burn_in_smoke --python $(PYTHON) --smoke --report-timeout-seconds 30

event-alpha-daily-live-no-send-burn-in-candidate-mode-smoke:
	RSI_EVENT_ALERTS_ENABLED=0 \
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.daily_burn_in --profile fixture --artifact-namespace daily_burn_in_candidate_mode_smoke --python $(PYTHON) --event-alpha-burn-in-candidate-mode --candidate-mode-smoke --report-timeout-seconds 30 --doctor-timeout-seconds 60

event-alpha-daily-live-no-send-burn-in-candidate-mode-smoke-doctor:
	RSI_EVENT_ALERTS_ENABLED=0 \
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.daily_burn_in --profile fixture --artifact-namespace daily_burn_in_candidate_mode_smoke --scoped-doctor
	RSI_EVENT_ALERTS_ENABLED=0 \
	$(PYTHON) main.py --event-alpha-artifact-doctor --event-alpha-profile fixture --event-alpha-artifact-namespace daily_burn_in_candidate_mode_smoke --event-alpha-include-test-artifacts --event-alpha-artifact-doctor-strict

event-alpha-daily-burn-in-readiness:
	RSI_EVENT_ALERTS_ENABLED=0 \
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.daily_burn_in --profile live_burn_in_no_send $(BURN_IN_NAMESPACE_ARG) --readiness-report

event-alpha-daily-review-inbox:
	RSI_EVENT_ALERTS_ENABLED=0 \
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.review_inbox --profile $(EVENT_ALPHA_RUNTIME_PROFILE) --artifact-namespace $(ARTIFACT_NAMESPACE)

event-alpha-burn-in-scorecard:
	RSI_EVENT_ALERTS_ENABLED=0 \
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.scorecard --profile $(BURN_IN_REPORT_PROFILE) $(BURN_IN_REPORT_NAMESPACE_ARG) --days 30 $(BURN_IN_POLICY_FLAGS) $(BURN_IN_COUNT_EXPLICIT_ARG)

event-alpha-feedback-progress:
	RSI_EVENT_ALERTS_ENABLED=0 \
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.feedback_progress --profile $(EVENT_ALPHA_RUNTIME_PROFILE) --artifact-namespace $(ARTIFACT_NAMESPACE) --days 30

event-alpha-burn-in-weekly-measurement:
	RSI_EVENT_ALERTS_ENABLED=0 \
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.measurement --profile $(BURN_IN_REPORT_PROFILE) $(BURN_IN_REPORT_NAMESPACE_ARG) --days 30 $(BURN_IN_POLICY_FLAGS)

event-alpha-source-yield-report:
	RSI_EVENT_ALERTS_ENABLED=0 \
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.operations.source_yield --profile $(BURN_IN_REPORT_PROFILE) $(BURN_IN_REPORT_NAMESPACE_ARG) --days 30 $(BURN_IN_POLICY_FLAGS)

conviction-priors-shadow-report:
	$(PYTHON) -m crypto_rsi_scanner.conviction_priors_shadow

paper-risk-research:
	$(PYTHON) -m crypto_rsi_scanner.paper_risk_research
...
event-alpha-shim-report: PROFILE = shim_report
event-alpha-shim-report:
	RSI_EVENT_ALERTS_ENABLED=0 \
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.shims --out-dir $(EVENT_ALPHA_PROFILE_DIR)

event-alpha-shim-dependency-report:
	RSI_EVENT_ALERTS_ENABLED=0 \
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.shims --dependency-report --out-dir research

event-alpha-old-import-check:
	RSI_EVENT_ALERTS_ENABLED=0 \
	$(PYTHON) -m crypto_rsi_scanner.event_alpha.shims --old-import-check --out-dir research
...
event-feedback-false-positive:
	RSI_EVENT_ALPHA_ARTIFACT_BASE_DIR=$(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
	RSI_EVENT_ALPHA_ARTIFACT_NAMESPACE=$(PROFILE) \
	$(PYTHON) main.py --event-feedback-false-positive "$(FEEDBACK_TARGET)" --event-feedback-notes "$(FEEDBACK_NOTES)" --event-alpha-profile $(PROFILE)

event-feedback-late:
	RSI_EVENT_ALPHA_ARTIFACT_BASE_DIR=$(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
	RSI_EVENT_ALPHA_ARTIFACT_NAMESPACE=$(PROFILE) \
	$(PYTHON) main.py --event-feedback-late "$(FEEDBACK_TARGET)" --event-feedback-notes "$(FEEDBACK_NOTES)" --event-alpha-profile $(PROFILE)

event-feedback-source-noise:
	RSI_EVENT_ALPHA_ARTIFACT_BASE_DIR=$(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
	RSI_EVENT_ALPHA_ARTIFACT_NAMESPACE=$(PROFILE) \
	$(PYTHON) main.py --event-feedback-source-noise "$(FEEDBACK_TARGET)" --event-feedback-notes "$(FEEDBACK_NOTES)" --event-alpha-profile $(PROFILE)

event-feedback-needs-confirmation:
	RSI_EVENT_ALPHA_ARTIFACT_BASE_DIR=$(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
	RSI_EVENT_ALPHA_ARTIFACT_NAMESPACE=$(PROFILE) \
	$(PYTHON) main.py --event-feedback-needs-confirmation "$(FEEDBACK_TARGET)" --event-feedback-notes "$(FEEDBACK_NOTES)" --event-alpha-profile $(PROFILE)

event-feedback-duplicate:
	RSI_EVENT_ALPHA_ARTIFACT_BASE_DIR=$(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
	RSI_EVENT_ALPHA_ARTIFACT_NAMESPACE=$(PROFILE) \
	$(PYTHON) main.py --event-feedback-duplicate "$(FEEDBACK_TARGET)" --event-feedback-notes "$(FEEDBACK_NOTES)" --event-alpha-profile $(PROFILE)

event-feedback-promising-source-type:
	RSI_EVENT_ALPHA_ARTIFACT_BASE_DIR=$(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
	RSI_EVENT_ALPHA_ARTIFACT_NAMESPACE=$(PROFILE) \
	$(PYTHON) main.py --event-feedback-promising-source-type "$(FEEDBACK_TARGET)" --event-feedback-notes "$(FEEDBACK_NOTES)" --event-alpha-profile $(PROFILE)
