...
help:
	...
	@echo "  make refactor-legacy-file-check  Fail if migration-era legacy files or flat Event Alpha shims remain"
...
test-full:
	...
	PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 $(PYTHON) -m pytest tests/event_alpha tests/rsi tests/cli tests/test_indicators.py -q
...
refactor-legacy-file-check:
	$(PYTHON) -m crypto_rsi_scanner.refactor_legacy_file_check
