...
PROFILE ?= no_key_live
EVENT_ALPHA_RUNTIME_PROFILE = $(if $(filter notify_llm_deep_rehearsal,$(PROFILE)),notify_llm_deep,$(PROFILE))
ALERT_KEY ?=
FEEDBACK_TARGET ?=
FEEDBACK_NOTES ?=
CONFIRM ?= 0
...
event-alpha-send-readiness:
	RSI_EVENT_ALPHA_ARTIFACT_BASE_DIR=$(EVENT_ALPHA_ARTIFACT_BASE_DIR) \
	RSI_EVENT_ALPHA_ARTIFACT_NAMESPACE=$(PROFILE) \
	$(if $(filter notify_llm_deep_rehearsal,$(PROFILE)),RSI_EVENT_ALERTS_ENABLED=0,) \
	$(PYTHON) main.py --event-alpha-send-readiness --event-alpha-profile $(EVENT_ALPHA_RUNTIME_PROFILE) --event-alpha-artifact-namespace $(PROFILE)
