...
.PHONY: test-pytest test-pytest-parallel test-event-alpha refactor-baseline event-alpha-namespace-lifecycle-report event-alpha-list-active-namespaces event-alpha-archive-stale-namespaces
...
test-pytest-parallel:
	@if ! $(PYTHON) -c "import pytest" >/dev/null 2>&1; then \
		...
		echo "pytest-xdist is not installed; running pytest without -n."; \
		$(PYTHON) -m pytest; \
	fi
