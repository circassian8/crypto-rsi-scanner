...
The funding arithmetic is closed for one explicitly supplied settlement, and
the pure interval v1 scenario can now aggregate a complete set relative to one
operator-supplied expected schedule. Bybit's official
...
flow plus its reconciliation to visible-book P&L. The interval projection also
requires strict timestamp ordering and exact equality between supplied event
times and the expected set, rejecting omissions, duplicates, additions, and
boundary ambiguity before aggregating cash flow. Because Bybit documents
symbol-specific `fundingInterval` minutes in
[instruments info](https://bybit-exchange.github.io/docs/v5/market/instrument),
the scenario requires a causal schedule reference and effective window covering
the modeled hold. It requires separate bounded rate and settlement-mark
references and causal observation clocks. It does not claim that Bybit's
...
have been captured, that the supplied expected schedule is authoritative, or
