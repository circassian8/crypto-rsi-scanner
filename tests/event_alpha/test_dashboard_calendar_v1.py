...
from crypto_rsi_scanner.event_alpha.dashboard.calendar_page import render_calendar_page
...
def test_calendar_groups_chronologically_and_renders_human_times_and_release_context():
    ...
    assert page.index("Tuesday, July 14") < page.index("Wednesday, July 15")
    assert page.index("Wednesday, July 15") < page.index("Monday, July 20")
...
def test_calendar_cards_are_semantic_responsive_surfaces_with_accessible_controls():
    ...
    assert '<form class="filter-panel calendar-filters"' in page
    ...
    assert 'class="calendar-grid"' in page or "No calendar events match these filters" in page
