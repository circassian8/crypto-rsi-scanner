"""Immutable KuCoin announcement capture boundary regressions."""
...
from crypto_rsi_scanner.event_alpha.operations.kucoin_announcements_capture import (
...
FIXTURE_DIR = Path(__file__).resolve().parents[2] / "fixtures" / "kucoin_announcements"


def _url(page: int) -> str:
    return (
        "https://api.kucoin.com/api/v3/announcements?"
        f"currentPage={page}&pageSize=50&annType=latest-announcements&"
        "lang=en_US&startTime=1784419200000&endTime=1784424900000"
    )


def _pages() -> tuple[KuCoinAnnouncementPageCapture, ...]:
    return (
        KuCoinAnnouncementPageCapture(
            page_number=1,
            request_url=_url(1),
...
def test_persistence_requires_explicit_confirmation(tmp_path: Path) -> None:
    with pytest.raises(
        KuCoinAnnouncementCaptureError, match="explicit_confirmation_required"
    ):
        persist_capture(
            tmp_path,
            _pages(),
...
def test_persist_and_strict_doctor_rederive_exact_artifacts(tmp_path: Path) -> None:
    ...
    assert ledger["requests"][0]["response_body_read_at"] == (
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("request_url", "https://example.com/api/v3/announcements", "capture_request_url_invalid"),
    ],
)
def test_transport_drift_fails_before_write(
    ...
    pages = list(_pages())
    row = pages[0]
    values = {**row.__dict__, field: value}
    pages[0] = KuCoinAnnouncementPageCapture(**values)

    with pytest.raises(KuCoinAnnouncementCaptureError, match=reason):
...
def test_response_mutation_fails_strict_doctor(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        KuCoinAnnouncementCaptureError, match="capture_ledger_response_invalid"
    ):
