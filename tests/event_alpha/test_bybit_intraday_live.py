...
def test_readiness_without_execution_capture_or_auth_is_zero_call_and_closed() -> None:
    ...
    summary = format_bybit_intraday_readiness_summary(payload)
    assert "report=decision_radar_bybit_intraday_readiness" in summary
    ...
    assert "provider_request_bound=0 (absolute=60)" in summary
    ...
    assert "RADAR_BYBIT_INTRADAY_READINESS_OUTPUT=json" in summary
...
def test_readiness_summary_fails_closed_on_request_count_drift() -> None:
    ...
    payload["maximum_provider_requests_for_current_capture"] = 3

    with pytest.raises(BybitIntradayLiveError, match="readiness_request_bound_mismatch"):
        format_bybit_intraday_readiness_summary(payload)
