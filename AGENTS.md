...
- Event-fade validation review bundles are artifact-only. `main.py
  ...
  `review_template.csv`, a gate-balanced `review_packet_balanced.md` and
  `review_template_balanced.csv` with proxy candidates and negative controls,
  reviewer helper columns including `source_search_url` for Google News/feed
  wrappers, a `review_guide.md` with label taxonomy, review provenance fields,
  and human event-time rules, and its README summarizes the same coverage,
